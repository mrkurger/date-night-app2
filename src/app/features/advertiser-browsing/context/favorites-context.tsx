...
export function FavoritesProvider({ children }: { children: React.ReactNode }) {
  ...
  const renameCollection = (collectionId: string, newName: string) => {
    setCollections(
      collections.map((collection) => (collection.id === collectionId ? { ...collection, name: newName } : collection)),
    )
  }
  ...
  const updateNote = (advertiserId: string, note: string) => {
    setNotes(
      notes.map((n) => (n.advertiserId === advertiserId ? { ...n, note, createdAt: new Date().toISOString() } : n)),
    )
  }
