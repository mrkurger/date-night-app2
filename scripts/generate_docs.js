...
let outputDir = null;
const outputDirIndex = args.findIndex(arg => arg.startsWith('--output-dir='));
if (outputDirIndex !== -1) {
  outputDir = args[outputDirIndex].split('=')[1];
  if (outputDir) {
    // Create the output directory if it doesn't exist
    if (!fs.existsSync(outputDir)) {
      fs.mkdirSync(outputDir, { recursive: true });
    }
    logVerbose(`Using output directory: ${outputDir}`);
  }
}
...
function createStubDocFile(targetDir, fileName, title, content) {
  // If outputDir is specified, adjust the target directory
  let finalTargetDir = targetDir;
  if (outputDir) {
    // Create a relative path structure in the output directory
    const relativeDir = path.relative(config.rootDir, targetDir);
    finalTargetDir = path.join(outputDir, relativeDir);
  ...
  // Don't overwrite existing files in the original location
  const originalFilePath = path.join(targetDir, fileName);
  if (fs.existsSync(originalFilePath) && !outputDir) {
    logVerbose(`File already exists: ${originalFilePath}`);
    return;
  }
...
function updateGlossaryFile(dir, functions) {
  // Determine the correct glossary path based on whether outputDir is specified
  let glossaryPath = path.join(dir, config.docFiles.glossary);
  let targetGlossaryPath = glossaryPath;

  if (outputDir) {
    // Create a relative path structure in the output directory
    const relativeDir = path.relative(config.rootDir, dir);
    const outputTargetDir = path.join(outputDir, relativeDir);
    ...
    targetGlossaryPath = path.join(outputTargetDir, config.docFiles.glossary);
  }

  // Read from the original location if it exists, otherwise create a new file
  let content;
  if (fs.existsSync(glossaryPath)) {
    content = fs.readFileSync(glossaryPath, 'utf8');
  } else {
    console.log(`Glossary file not found: ${glossaryPath}, creating a new one`);
    const dirName = path.basename(dir);
    const title = dirName.charAt(0).toUpperCase() + dirName.slice(1).replace(/-/g, ' ');
...
<body>
  ...
  fs.writeFileSync(targetGlossaryPath, content);
  console.log(`Updated glossary file: ${targetGlossaryPath}`);
}
...
function updateGlobalGlossary(allFunctions) {
  // Determine the correct global glossary path based on whether outputDir is specified
  let globalGlossaryPath;

  if (outputDir) {
    globalGlossaryPath = path.join(outputDir, config.globalDocs.glossary);
  } else {
    globalGlossaryPath = path.join(config.rootDir, config.globalDocs.glossary);
  }
