  ...
  return new Promise(resolve => {
    exec('mongod --version', (error, stdout, stderr) => {
      if (error) {
        ...
        console.log(
          `${colors.yellow}Please install MongoDB: https://www.mongodb.com/try/download/community${colors.reset}`
        );
        resolve(false);
      } else {
        ...
        if (versionMatch && versionMatch[1]) {
          ...
          console.log(
            `${colors.yellow}MongoDB is installed but version could not be determined${colors.reset}`
          );
          resolve(true);
        }
      }
...
function checkDataDirectory() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `${colors.green}Current user has write permissions to the data directory${colors.reset}`
      );
    } catch (err) {
      console.log(
        `${colors.red}Current user does not have write permissions to the data directory${colors.reset}`
      );
      ...
      try {
        if (os.platform() !== 'win32') {
          ...
          console.log(
            `${colors.yellow}On Windows, please run the application as administrator or manually set permissions${colors.reset}`
          );
        }
      } catch (err) {
    ...
    console.log(
      `${colors.red}Failed to check directory permissions: ${err.message}${colors.reset}`
    );
    return false;
  }
...
function checkLockFiles() {
  ...
  if (fs.existsSync(lockFile)) {
    ...
    console.log(
      `${colors.yellow}This may indicate MongoDB did not shut down properly${colors.reset}`
    );
...
function startMongoDBVerbose() {
  ...
  return new Promise(resolve => {
    const mongod = spawn('mongod', [
      ...
      '--fork',
      '--logpath',
      path.join(process.cwd(), 'mongodb.log'),
      '--logappend',
    ]);
    ...
    mongod.on('close', code => {
      if (code === 0) {
        ...
        console.log(
          `${colors.blue}Log file: ${path.join(process.cwd(), 'mongodb.log')}${colors.reset}`
        );
        resolve(true);
      } else {
...
async function main() {
  ...
  if (!isInstalled) {
    console.log(
      `${colors.red}MongoDB is not installed. Please install MongoDB first.${colors.reset}`
    );
    process.exit(1);
  }
  ...
  if (isRunning) {
    console.log(
      `${colors.yellow}MongoDB is already running. Stopping it for diagnostics...${colors.reset}`
    );
    try {
      if (os.platform() === 'win32') {
      ...
      console.log(
        `${colors.yellow}Please stop MongoDB manually and run this script again${colors.reset}`
      );
      process.exit(1);
    }
  }
  ...
  if (!repairOk) {
    console.log(
      `${colors.yellow}Database repair had issues, but we'll try to start MongoDB anyway${colors.reset}`
    );
  }
  ...
  if (startOk) {
    ...
    console.log(
      `${colors.yellow}Please check the MongoDB log file for more details${colors.reset}`
    );
    process.exit(1);
  }
}
