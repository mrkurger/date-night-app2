...
try {
  ...
  if (os.platform() !== 'win32') {
    try {
      ...
      console.log(
        `${colors.yellow}Could not determine current user/group: ${err.message}${colors.reset}`
      );
    }
  }
  ...
  if (os.platform() !== 'win32') {
    ...
    try {
      ...
      if (owner !== currentUser) {
        console.log(
          `${colors.yellow}Warning: The directory is not owned by the current user.${colors.reset}`
        );
      }
    } catch (err) {
    ...
    console.log(
      `${colors.blue}Permissions: Windows permissions model (not displayed)${colors.reset}`
    );
  }
    ...
    if (os.platform() !== 'win32') {
      try {
        ...
        if (process.getuid && process.getuid() === 0) {
          ...
          console.log(
            `${colors.green}Changed ownership to ${currentUser}:${currentGroup}${colors.reset}`
          );
        } else {
          console.log(
            `${colors.yellow}Not running as root, cannot change ownership.${colors.reset}`
          );
          console.log(
            `${colors.yellow}You may need to run: sudo chown -R ${currentUser}:${currentGroup} "${absoluteDbPath}"${colors.reset}`
          );
        }
        ...
        try {
          ...
          try {
            ...
            console.log(
              `${colors.red}Delete test after fix: Failed - ${err.message}${colors.reset}`
            );
          }
        } catch (err) {
          console.log(`${colors.red}Write test after fix: Failed - ${err.message}${colors.reset}`);
          console.log(
            `${colors.red}You may need to manually fix permissions or run this script with sudo.${colors.reset}`
          );
        }
      } catch (err) {
      ...
      console.log(
        `${colors.yellow}On Windows, you need to ensure the current user has full control of the directory.${colors.reset}`
      );
      console.log(
        `${colors.yellow}Right-click the folder, select Properties > Security > Edit, and add your user with Full Control.${colors.reset}`
      );
    }
  }
  ...
  if (os.platform() !== 'win32') {
    console.log(
      `${colors.magenta}1. Ensure MongoDB data directory has permissions 755 (rwxr-xr-x)${colors.reset}`
    );
    console.log(
      `${colors.magenta}2. Ensure the directory is owned by the user running the application${colors.reset}`
    );
    console.log(
      `${colors.magenta}3. If using MongoDB from a package manager, you may need to add your user to the 'mongodb' group:${colors.reset}`
    );
    console.log(`${colors.magenta}   sudo usermod -aG mongodb ${currentUser}${colors.reset}`);
  } else {
    console.log(
      `${colors.magenta}1. Ensure your user account has Full Control permissions on the data directory${colors.reset}`
    );
    console.log(
      `${colors.magenta}2. Run the application as Administrator if you continue to have issues${colors.reset}`
    );
  }
