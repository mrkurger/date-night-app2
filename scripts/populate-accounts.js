...
function getRandomPricing() {
  ...
  for (let i = 0; i < numPrices; i++) {
    ...
    pricing.push({
      ...
      duration,
      customDuration:
        duration === 'custom' ? `${Math.floor(Math.random() * 5) + 3} hours` : undefined,
    });
  }
  ...
  for (let i = 0; i < numItineraries; i++) {
    ...
    itineraries.push({
      ...
      accommodation: {
        ...
        address:
          Math.random() > 0.7
            ? `${Math.floor(Math.random() * 100) + 1} Main St, ${city}`
            : undefined,
        location:
          Math.random() > 0.5
            ? {
      ...
      notes:
        Math.random() > 0.7
          ? `Visiting ${city} for a short stay. Available for bookings.`
          : undefined,
      status: 'planned',
    });
  }
...
function generateRandomAd(advertiserId) {
  ...
  return {
    ...
    address: {
      street:
        Math.random() > 0.7 ? `${Math.floor(Math.random() * 100) + 1} ${city} Street` : undefined,
    ...
    contact: {
      ...
      email:
        Math.random() > 0.5 ? `contact${Math.floor(Math.random() * 1000)}@example.com` : undefined,
      website:
        Math.random() > 0.8 ? `https://example.com/${Math.floor(Math.random() * 1000)}` : undefined,
...
function generateRandomUser(role, index) {
  ...
  const firstName =
    gender === 'female'
      ? ['Sofia', 'Emma', 'Olivia', 'Nora', 'Ella', 'Maja', 'Ingrid', 'Sara', 'Amalie', 'Leah'][
          Math.floor(Math.random() * 10)
        ]
      : ['Liam', 'Noah', 'Oliver', 'Emil', 'Oskar', 'Elias', 'Aksel', 'Mathias', 'Magnus', 'Jakob'][
          Math.floor(Math.random() * 10)
        ];
