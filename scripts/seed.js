...
async function seedDatabase() {
  console.log('This script requires bcrypt package to be installed');
  ...
  try {
    ...
    await Promise.all([db.collection('users').deleteMany({}), db.collection('ads').deleteMany({})]);
    ...
    for (const advertiserId of advertiserDocs) {
      ...
      for (let i = 0; i < numAds; i++) {
        ...
        ads.push({
          ...
          location: {
            type: 'Point',
            coordinates: [5.322 + (Math.random() - 0.5) * 2, 60.3913 + (Math.random() - 0.5) * 2],
          },
          active: true,
