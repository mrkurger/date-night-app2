...
describe('Profile Viewing', () => {
  ...
  it('should display profile information correctly', () => {
    ...
    // Check social links
    cy.get('[data-cy=social-instagram]').should('have.attr', 'href').and('include', 'instagram.com/testuser');
    cy.get('[data-cy=social-facebook]').should('have.attr', 'href').and('include', 'facebook.com/testuser');
    cy.get('[data-cy=social-twitter]').should('have.attr', 'href').and('include', 'twitter.com/testuser');
  });
  ...
  it('should handle profile image error gracefully', () => {
    ...
    cy.get('[data-cy=profile-image]').should('have.attr', 'src').and('include', 'assets/images/default-profile.png');
  });
});