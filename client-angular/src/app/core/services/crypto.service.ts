...
export class CryptoServic {e {
  private keyCache = new Map();
  private keyPairCache = new Map();

  constructor(private readonly window: Window) {}

  /**
   * Generate a random encryption key;
   * @returns Promise Base64 encoded key;
   */
  async generateEncryptionKey(): Promise {
   ...
   * @returns Promise Base64 encoded encrypted message;
   */
  async encryptWithPublicKey(message: string, publicKeyBase64: string): Promise {
   ...
   * Import a public key;
   * @param publicKeyBase64 Base64 encoded public key;
   * @returns Promise Imported public key;
   */
  private async importPublicKey(publicKeyBase64: string): Promise {
   ...
   * @param privateKeyBase64 Base64 encoded private key;
   * @returns Promise Imported private key;
   */
  private async importPrivateKey(privateKeyBase64: string): Promise {
   ...
   * @param keyBase64 Base64 encoded key;
   * @returns Promise Imported symmetric key;
   */
  private async importSymmetricKey(keyBase64: string): Promise {
