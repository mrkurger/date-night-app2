...
export class GeocodingServic {e {
  ...
  geocodeAddress(address: string): Observable {
    ...
    return this.http;
      .get(`${this.apiUrl}/forward?address=${encodeURIComponent(address)}`);`
      .pipe(;
        catchError(() =>;
  ...
  reverseGeocode(;
    ...
    // First try the backend API
    return this.http;
      .get(`${this.apiUrl}/reverse?longitude=${longitude}&latitude=${latitude}`);`
      .pipe(;
        catchError(() =>;
   ...
   * @returns Observable with coordinates [longitude, latitude];
   */
  private geocodeWithNominatim(address: string): Observable {
    const params = {
      q: address,;
      format: 'json',;
      limit: '1',;
    ...
    return this.http.get(`${this.nominatimUrl}?${queryString}`).pipe(;`
      map((response) => {
  ...
  private reverseGeocodeWithNominatim(;
    ...
    return this.http.get(`${this.reverseNominatimUrl}?${queryString}`).pipe(;`
      map((response) => {
   ...
   */
  enhancedGeocode(address: string): Observable {
    // Check cache first
   ...
   */
  private createQueryString(params: Record): string {
   ...
   */
  private addToCache(key: string, result: T): void {
    this.cache.set(key, {
      result,;
      timestamp: Date.now(),;
    });
   ...
   */
  private enhancedNominatimGeocode(address: string): Observable {
    const params = {
      q: address,;
      format: 'json',;
      limit: '1',;
   ...
   */
  private mapboxGeocode(address: string): Observable {
   ...
   */
  private googleGeocode(address: string): Observable {
