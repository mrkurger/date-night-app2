  ...
  applyFilters(): void {
    ...
    this.filteredAds = this.ads.filter(ad => {
      ...
      if (filters.categories) {
        const categorySelected = filters.categories.escort || 
                                filters.categories.massage || 
                                filters.categories.striptease;
  ...
  getActiveFilters(): FilterItem[] {
    ...
    if (filters.categories) {
      ...
      if (filters.categories.striptease) {
        activeFilters.push({ key: 'categories.striptease', value: true, label: 'Category: Striptease' });
      }
    }
    ...
    if (filters.dateRange) {
      if (filters.dateRange.from) {
        ...
        activeFilters.push({ key: 'dateRange.from', value: filters.dateRange.from, label: `From: ${fromDate}` });
      }
      if (filters.dateRange.to) {
        const toDate = new Date(filters.dateRange.to).toLocaleDateString();
        activeFilters.push({ key: 'dateRange.to', value: filters.dateRange.to, label: `To: ${toDate}` });
      }
    }
    ...
    // Add search query
    if (filters.searchQuery) {
      activeFilters.push({ key: 'searchQuery', value: filters.searchQuery, label: `Search: ${filters.searchQuery}` });
    }
