...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      ...
      <!-- Bank Account Form -->
      <form *ngIf="selectedPaymentType === 'bank_account'" [formGroup]="bankForm" class="payment-form">
        <mat-form-field appearance="outline" class="full-width">
      ...
      <form *ngIf="selectedPaymentType === 'crypto_address'" [formGroup]="cryptoForm" class="payment-form">
        <mat-form-field appearance="outline" class="full-width">
        ...
        <mat-form-field appearance="outline" class="full-width">
          ...
          <mat-select formControlName="network">
            <mat-option *ngFor="let network of getNetworksForCurrency(cryptoForm.get('currency')?.value)" [value]="network.value">
              {{ network.label }}
            </mat-option>
          </mat-select>
        ...
        <mat-form-field appearance="outline" class="full-width" *ngIf="requiresMemo(cryptoForm.get('currency')?.value)">
    ...
    <mat-dialog-actions align="end">
      ...
      <button 
        ...
        color="primary" 
        [disabled]="
          (selectedPaymentType === 'card' && (cardForm.invalid || !cardComplete || processingAddPaymentMethod)) || 
          (selectedPaymentType === 'bank_account' && (bankForm.invalid || processingAddPaymentMethod)) || 
          (selectedPaymentType === 'crypto_address' && (cryptoForm.invalid || processingAddPaymentMethod))
        " 
...
export class AddPaymentMethodDialogComponent implements OnInit {
  ...
  addPaymentMethod(): void {
    ...
    if (this.selectedPaymentType === 'card') {
      ...
      const { accountHolderName, bankName, accountNumber, routingNumber, accountType, currency, setAsDefault } = this.bankForm.value;
    ...
    this.walletService.addPaymentMethod(paymentMethodData)
      .subscribe({
        next: (paymentMethod) => {
