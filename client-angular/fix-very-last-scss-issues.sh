...
# This script fixes the very last SCSS issues

echo "Fixing very last SCSS issues..."
...
if [ -f "$PAGER_FILE" ]; then
  ...
  sed -i '' 's/\$font-size-base/1rem/g' "$PAGER_FILE"
fi
...
if [ -f "$FAB_FILE" ]; then
  ...
  sed -i '' 's/\$font-size-base/1rem/g' "$FAB_FILE"
fi
...
if [ -f "$INFO_PANEL_FILE" ]; then
  ...
  sed -i '' 's/emerald\.\$color-success-700: #15803d;/\$success-dark: #15803d;/g' "$INFO_PANEL_FILE"
  sed -i '' 's/emerald\.\$color-warning-700: #a16207;/\$warning-dark: #a16207;/g' "$INFO_PANEL_FILE"
...
if [ -f "$CARD_GRID_FILE" ]; then
  ...
  sed -i '' 's/box-shadow: \$shadow-sm;/box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05);/g' "$CARD_GRID_FILE"
fi
...
if [ -f "$AVATAR_FILE" ]; then
  ...
  sed -i '' 's/\$font-size-xs/0.75rem/g' "$AVATAR_FILE"
fi
...
if [ -f "$TINDER_FILE" ]; then
  ...
  sed -i '' 's/emerald\.\$color-primary/tokens.\$color-primary/g' "$TINDER_FILE"
  sed -i '' 's/emerald\.\$color-secondary/tokens.\$color-secondary/g' "$TINDER_FILE"
fi
...
if [ -f "$PAGE_HEADER_FILE" ]; then
  ...
  sed -i '' 's/\$font-weight-bold/700/g' "$PAGE_HEADER_FILE"
fi
...
if [ -f "$DESIGN_SYSTEM_DEMO_FILE" ]; then
  ...
  sed -i '' 's/ds\.\$color-primary/#3b82f6/g' "$DESIGN_SYSTEM_DEMO_FILE"
fi

echo "All very last SCSS issues fixed!"