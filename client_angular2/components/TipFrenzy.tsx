...
export function TipFrenzy() {
  ...
  const handleTip = (amount: number, xpGain: number) => {
    ...
    const randomAdvertiser =
      targetAdvertisers[Math.floor(Math.random() * targetAdvertisers.length)];

    if (randomAdvertiser) {
      setRecentTips(prev => [
        { id: randomAdvertiser.id, amount, name: randomAdvertiser.name },
        ...prev.slice(0, 4),
      ]);
    }
        ...
        <CardContent className="p-4">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex -space-x-2">
              {(frenzyMode === 'selected' ? selectedAdvertisers : randomAdvertisers)
                .slice(0, 6)
                .map(advertiser => (
            ...
            {frenzyMode === 'random' && (
              <Button
                size="sm"
                variant="outline"
                onClick={shuffleRandomAdvertisers}
                className="text-xs"
              >
                <Shuffle className="h-3 w-3 mr-1" />
                Shuffle
          ...
          <div className="grid grid-cols-7 gap-2 mb-4">
            {tipOptions.map(option => (
              <Button
                ...
                className={`${
                  option.color
                } hover:scale-105 transition-all duration-200 text-white border-0 flex flex-col py-3 h-auto ${
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-700">
            <div className="text-xs text-gray-400">
              Tip {frenzyMode === 'selected' ? 'selected' : 'random'} advertisers • Earn XP • Get
              exclusive content
            </div>
