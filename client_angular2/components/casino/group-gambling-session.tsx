        ...
        <div className="lg:col-span-3 space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                    ...
                    <span>•</span>
                    <span>{participants.length}/{session.maxParticipants} Players</span>
                    <span>•</span>
                    <span className="capitalize">{session.gameType}</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-primary">{session.totalPot.toLocaleString()} NOK</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card className="bg-card/50 backdrop-blur-sm border-primary/20">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-2xl font-bold text-blue-500">{participants.filter(p => p.isOnline).length}</div>
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-primary/20">
            <CardContent className="p-4">
              ...
              <Progress value={(session.currentRound / session.totalRounds) * 100} className="h-2" />
            </CardContent>
          </Card>
                  ...
                  <div className="flex gap-2">
                    {[100, 250, 500, 1000].map(amount => (
                      <Button
                        ...
                        className={currentBet === amount ? 'bg-primary text-primary-foreground' : ''}
                      >
                        {amount}
                      </Button>
                    ))}
                    ...
                    <div key={message.id} className="text-sm">
                      <div className="flex items-center gap-2">
                        <span className={`font-semibold ${
                          ...
                          message.type === 'bet' ? 'text-orange-500' :
                          message.type === 'win' ? 'text-green-500' :
                          'text-primary'
                        }`}>
                          {message.userName}:
                        </span>
