...
import { Dice1, Heart, Diamond, Spade, Club, Trophy, Gift, DollarSign, Timer, Star } from "lucide-react"
...
const advertisers: Advertiser[] = [
  ...
  },
  {
    id: 4,
    name: "Olivia",
    image: "https://images.unsplash.com/photo-1488716820095-cbe80883c496?w=100&h=100&fit=crop&crop=face",
    cardRank: "Jack",
    cardSuit: "Clubs",
    totalEarnings: 18000,
    winRate: 0.75,
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-900 to-red-900 p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card className="bg-black/40 backdrop-blur-sm border-pink-500/30 p-4">
          ...
          <div className="text-2xl font-bold text-yellow-400">${currentJackpot.toLocaleString()}</div>
        </Card>
        ...
        <Card className="bg-black/40 backdrop-blur-sm border-pink-500/30 p-4">
          ...
          <div className="text-2xl font-bold text-green-400">{activeRaffles}</div>
        </Card>
      ...
      <Tabs defaultValue="keno" className="w-full">
        ...
        <TabsContent value="keno" className="mt-6">
          <KenoGame advertisers={advertisers} userBalance={userBalance} setUserBalance={setUserBalance} />
        </TabsContent>

        {/* Card Games */}
        <TabsContent value="cards" className="mt-6">
          <CardGames advertisers={advertisers} userBalance={userBalance} setUserBalance={setUserBalance} />
        </TabsContent>

        {/* Active Raffles */}
        <TabsContent value="raffles" className="mt-6">
          <ActiveRaffles advertisers={advertisers} userBalance={userBalance} setUserBalance={setUserBalance} />
        </TabsContent>
              ...
              >
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-1 mt-2">
                    ...
                    <span className="text-yellow-400 text-xs">{(advertiser.winRate * 100).toFixed(0)}%</span>
                  </div>
                </div>
              </Card>
            ))}
      ...
      {raffles.map((raffle) => (
        <Card key={raffle.id} className="bg-black/40 backdrop-blur-sm border-pink-500/30 p-6">
          <div className="text-center mb-4">
            <img
              src={raffle.advertiser.image || "/placeholder.svg"}
    ...
    <Card className="bg-black/40 backdrop-blur-sm border-pink-500/30 p-6">
      ...
      <div className="space-y-4">
        {sortedAdvertisers.map((advertiser, index) => (
          <div key={advertiser.id} className="flex items-center gap-4 p-4 bg-gray-800/50 rounded-lg">
            ...
            <div className="text-right">
              <div className="text-green-400 font-bold">${advertiser.totalEarnings.toLocaleString()}</div>
              <div className="text-gray-400 text-sm">Total Earnings</div>
            </div>

            <div className="text-right">
              <div className="text-blue-400 font-bold">{(advertiser.winRate * 100).toFixed(0)}%</div>
