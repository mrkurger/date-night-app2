...
export function MasonryGrid({ advertisers, className }: MasonryGridProps) {
  ...
  const gridAdvertisers = useMemo(() => {
    const initialAdvertisers = advertisers.slice(0, Math.min(7, advertisers.length));
    const filledAdvertisers = [...initialAdvertisers];
    while (filledAdvertisers.length < 7) {
      filledAdvertisers.push({
        ...initialAdvertisers[filledAdvertisers.length % initialAdvertisers.length],
      });
    }
    return filledAdvertisers;
  }, [advertisers]);
