...
import argon2 from 'argon2'; // Added argon2 import
...
export interface IUserDocument extends IUser, Document {
  comparePassword(_candidatePassword: string): Promise<boolean>; // Prefixed unused
...
userSchema.methods.comparePassword = async function (
  ...
  if (!this.password) {
    // Handle case where password is not set, though schema requires it.
    // This might indicate a state where the document is not yet saved or populated.
    return false;
  }
...
userSchema.methods.getActiveTravelPlans = function (this: IUserDocument): ITravelPlanItem[] {
  // const now = new Date();
  // return this.travelPlan && this.travelPlan.filter(plan => plan.endDate && plan.endDate >= now && plan.startDate <= now);
  if (!this.travelPlan) {
    return [];
  }
  return this.travelPlan.filter((plan: ITravelPlanItem) => plan.active === true);
};

// Method to get upcoming travel plans
userSchema.methods.getUpcomingTravelPlans = function (this: IUserDocument): ITravelPlanItem[] {
  // const now = new Date();
  // return this.travelPlan && this.travelPlan.filter(plan => plan.startDate && plan.startDate > now);
  if (!this.travelPlan) {
    return [];
  }
  const now = new Date();
  return this.travelPlan.filter(
    (plan: ITravelPlanItem) => plan.endDate > now && plan.active === true
  );
};
