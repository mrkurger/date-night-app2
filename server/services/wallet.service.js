...
class WalletService {
      ...
      switch (paymentMethodData.type) {
        ...
        case 'crypto_address':
          ...
          if (!SUPPORTED_CRYPTOCURRENCIES.includes(paymentMethodData.cryptoDetails.currency)) {
            throw new AppError(`Unsupported cryptocurrency: ${paymentMethodData.cryptoDetails.currency}`, 400);
          }
          break;
   ...
   */
  async depositFundsWithStripe(userId, amount, currency, paymentMethodId, description = 'Wallet deposit') {
    try {
      // Validate inputs
      if (!amount || amount <= 0) {
   ...
   */
  async withdrawFunds(userId, amount, currency, paymentMethodId, description = 'Wallet withdrawal') {
    try {
      // Validate inputs
      if (!amount || amount <= 0) {
        throw new AppError('Valid amount is required', 400);
      ...
      if (amount < MINIMUM_WITHDRAWAL) {
        throw new AppError(`Minimum withdrawal amount is ${MINIMUM_WITHDRAWAL / 100} ${currency}`, 400);
      }
      ...
      if (balance.available < amount) {
        throw new AppError(`Insufficient balance. Available: ${balance.available / 100} ${currency}`, 400);
      }
      ...
      setTimeout(async () => {
        try {
          ...
          const pendingTransaction = updatedWallet.transactions.find(
            t => t.status === 'pending' && t.type === 'withdrawal' && t._id.toString() === transaction._id.toString()
          );
   ...
   * @returns {Promise<Object>} Withdrawal transaction
   */
  async withdrawCrypto(userId, amount, currency, address, network, memo, description = 'Crypto withdrawal') {
    try {
      // Validate inputs
      if (!amount || amount <= 0) {
        throw new AppError('Valid amount is required', 400);
      ...
      if (balance.available < amount) {
        throw new AppError(`Insufficient balance. Available: ${balance.available} ${currency}`, 400);
      }
      ...
      if (balance.available < amount + cryptoFeeAmount) {
        throw new AppError(`Insufficient balance after fee. Available: ${balance.available} ${currency}, Required: ${amount + cryptoFeeAmount} ${currency}`, 400);
      }
   ...
   */
  async transferFunds(senderUserId, recipientUserId, amount, currency, description = 'Wallet transfer') {
    try {
      // Validate inputs
      if (!amount || amount <= 0) {
        throw new AppError('Valid amount is required', 400);
      ...
      if (!SUPPORTED_CURRENCIES.includes(currency) && !SUPPORTED_CRYPTOCURRENCIES.includes(currency)) {
        throw new AppError(`Unsupported currency: ${currency}`, 400);
      }
      ...
      if (senderBalance.available < amount) {
        throw new AppError(`Insufficient balance. Available: ${senderBalance.available / 100} ${currency}`, 400);
      }
  ...
  async updateWalletSettings(userId, settings) {
    try {
      ...
      if (settings.defaultCurrency) {
        if (!SUPPORTED_CURRENCIES.includes(settings.defaultCurrency) && 
