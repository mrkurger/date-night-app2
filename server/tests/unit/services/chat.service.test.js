...
jest.mock('../../../middleware/errorHandler.js', () => ({ // Mock AppError specifically if needed
...
describe('Chat Service', () => {
  ...
  beforeEach(() => {
    ...
    ChatMessage.create = jest.fn().mockImplementation(data => Promise.resolve({
      ...data,
      _id: 'mock-message-id',
    ...
    ChatRoom.findById = jest.fn().mockReturnValue({ // Make it chainable for populate
    ...
    Ad.findById = jest.fn().mockReturnValue({ // Make it chainable for populate
    ...
    cryptoHelpers.generateKeyPair = jest.fn().mockReturnValue({ publicKey: 'newPublicKey', privateKey: 'newPrivateKey' });
  ...
  describe('sendMessage', () => {
    ...
    it('should throw AppError when room not found', async () => {
      ...
      await expect(
        chatService.sendMessage(mockRoomId, mockUserId1, 'Hello')
  ...
  describe('getMessages', () => {
    it('should get messages for a chat room with default options', async () => {
      const mockMessagesData = [{ _id: 'msg1', message: 'Hi' }, { _id: 'msg2', message: 'There' }];
    ...
    it('should throw AppError if database query fails', async () => {
        ...
        ChatMessage.find.mockImplementation(() => { throw dbError; }); // Make find throw
  ...
  describe('createDirectRoom', () => {
      ...
      const participantsPopulateMock = jest.fn().mockReturnValue({ populate: lastMessagePopulateMock });
    ...
    it('should throw AppError if findOrCreateDirectRoom fails', async () => { // Reverted to expect().rejects
        ...
        const dbError = new Error('DB connection failed');
          ...
          await expect(chatService.createDirectRoom(mockUserId1, mockUserId2)).rejects.toThrow('DB connection failed');
      });
  });
  ...
  describe('getRoomsForUser', () => {
    it('should return rooms for a user with unread counts', async () => { // Added async
     ...
     it('should throw AppError if user not found', async () => { // Added async
   ...
   describe('markMessagesAsRead', () => {
    it('should mark messages as read and update room last read', async () => { // Added async
     ...
     it('should throw AppError if room not found', async () => { // Added async
