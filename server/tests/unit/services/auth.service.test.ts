...
jest.mock('../../../models/session.model.js', () => {
  ...
  const MockSession = jest.fn().mockImplementation(sessionData => ({
    ...sessionData,
    _id: new mongooseLocal.Types.ObjectId(),
    userId: new mongooseLocal.Types.ObjectId(),
    ipAddress: '127.0.0.1',
  ...
  MockSession.findById = jest.fn().mockResolvedValue(null); // Default to not finding a session
...
jest.mock('../../../models/user.model.js', () => {
  ...
  MockUser.create = jest.fn().mockImplementation(async userData => {
    // Simulate create returning a saved instance
    const instance = MockUser(userData);
    // Mongoose create() typically returns a promise that resolves with the saved document.
    // The instance created by MockUser should already have a .save() method mocked
    // and its data.
    return Promise.resolve(instance);
  });

  return { __esModule: true, default: MockUser }; // Return the mocked constructor as default export
});

// Import the modules after mocking
import authService from '../../../services/auth.service.js';
import User from '../../../models/user.model.js'; // This will be the mocked User
import Session from '../../../models/session.model.js'; // This will be the mocked Session
