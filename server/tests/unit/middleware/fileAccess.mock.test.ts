...
// Create interfaces for our mock objects
...
// Interface to extend Request with validatedFilePath
...
// Mock middleware functionality
async function fileAccessMock(
  req: MockRequest,
  res: MockResponse,
  next: NextFunction
): Promise<void> {
  try {
    ...
    try {
      ...
      // Check file size
      const stats = (await mockStat(fullPath)) as Stats;
