...
describe('ChatRoom Model', () => {
  ...
  describe('Basic Validation', () => {
    ...
    it('should require participants with valid user references', async () => {
      ...
      await expect(roomWithoutParticipants.save()).rejects.toThrow();
    });
    ...
    it('should validate participant role', async () => {
      const roomWithInvalidRole = new ChatRoom({
        type: 'group',
        participants: [
          { user: user1._id, role: 'invalid-role' },
          { user: user2._id }
        ],
        createdBy: user1._id
      });
  ...
  describe('Instance Methods', () => {
    ...
    it('should add a participant with specified role', async () => {
      ...
      const participant = testRoom.participants.find(p => 
        p.user.toString() === user3._id.toString()
      );
  ...
  describe('Static Methods', () => {
    ...
    it('should not return inactive rooms', async () => {
      ...
      expect(user1Rooms[0].participants[1].user.toString()).toBe(user2._id.toString());
    });
  });
});